...
void checkTimeout() {
    ...
    if (!disableTimeout) {
        if (CLOCK().getTimeDiff() >= TIMEOUT * 1000) {
            ...
            switch (CurrentAppState) {
                ...
                #if !OTA_APP
                case TXT:
                if (SLEEPMODE == "TEXT" && PM_SD().getEditingFile() != "" && !OTA_APP) {
                    /*
    ...
    if (PWR_BTN_event && CurrentHOMEState != NOWLATER) {
        ...
        prefs.putString("editingFile", PM_SD().getEditingFile());
        prefs.end();
        ...
        display.fillScreen(GxEPD_WHITE);
            ...
            switch (CurrentAppState) {
                ...
                if (SLEEPMODE == "TEXT" && PM_SD().getEditingFile() != "" && !OTA_APP) {
                    ...
                    //display.drawBitmap(display.width() - 30, display.height() - 20, KBStatusallArray[6], 30,
                    //                20, GxEPD_BLACK);
                    EINK().statusBar(PM_SD().getEditingFile(), true);
...
void loadState(bool changeState) {
    ...
    PM_SD().setEditingFile(prefs.getString("editingFile", ""));
...
void saveEditingFile() {
    ...
    if (!OTA_APP){
        ...
        if (savePath != "" && savePath != "-" && savePath != "/temp.txt" && fileLoaded) {
            ...
            saveMarkdownFile(PM_SD().getEditingFile());
